...
class MobileNetV2_3x3_Product(nn.Module):
    """
    Should freeze bn
    """
    def __init__(self, n_class=1000, input_size=224, width_mult=1.):
        super(MobileNetV2_3x3_Product, self).__init__()
    ...
    def forward(self, x):
        ...
        for i, origin_layer in enumerate(self.features):
            if i in self.return_features_indices:
                ...
                pb_weight = F.adaptive_avg_pool2d(pb_out, 1)
                pb_weight = torch.sigmoid(pb_weight)
                # Then calculate output of origin backbone.
                origin_out = origin_layer(x)
                # Make product operation
                x = pb_weight * origin_out
...
# net = MobileNetV2_3x3_Product()
# print(net(data), net(data).shape)
